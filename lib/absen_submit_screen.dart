...
class _AbsenSubmitScreenState extends State<AbsenSubmitScreen> {
  ...
  Future<void> _cancelCamera() async {  // ← Tambahkan async
  cam.hidePreview();
  ...
  Future<void> _submitAbsen() async {
    ...
    try {
      ...
      if (res.statusCode == 200 || res.statusCode == 201) {
        if (mounted) {
          // ✅ Tutup dialog success dan balik dengan flag refresh
          ...
          // Pop dua kali untuk balik ke list kehadiran
          if (mounted) {
            Navigator.pop(context); // Pop AbsenSubmitScreen
            Navigator.pop(context, true); // Pop ke KehadiranScreen dengan refresh flag
          }
        }
      } else {
        throw DioException(
  ...
  Widget build(BuildContext context) {
    return Stack(
      children: [
        ...
        // ✅ FLUTTER OVERLAY UNTUK CAMERA CONTROLS
        if (showingCamera)
          Positioned.fill(
            child: Container(
              color: Colors.black,
              child: Column(
                children: [
                  ...
                  // Camera controls at bottom - BIGGER HIT AREA
                  Container(
                    width: double.infinity,
